...
statusColor <- c("forestgreen", "orange", "red")
```
...
multivariate <- function(x,y,z, log = F, names=c('x','y','z')){
  df <- data.frame(x = x, y=y, z=z)
  p <- ggplot(subset(df,y>0),
              aes(x, y)) + 
    scale_fill_manual(values = statusColor, name=names[3]) + 
  ...
  p1 <- p + geom_boxplot()
  p2 <- p + geom_boxplot(aes(fill=z))
  multiplot(p1,p2,cols=1)
}

multipie <- function(x,y,z){
  df <- data.frame(x=x, y=y, z=z)
  tab <- as.data.frame(table(df))
  tab <- melt(tab)

  ggplot(tab, aes(x="", y= value, fill = z)) +
    facet_grid(x~y) +
    geom_bar(stat = "identity", width = 1, position = position_fill()) + coord_polar(theta = "y")
}
```
...
The data is extremely skewed with mostly zeros, which probably represent NAs. Thus here they are plotted again with the zeros removed and with a log-scaled x-axis:
...
It seems that low numbers are slightly more common.The description of the data variables on the drivendata.org website does not explain the meaning of this variable. Intuitively it might be an internal id and thus is probably worthless for prediction. 
...
p
```
This variable represents the name of the water point. Most data points were recorded just once, but some names are used up to seven times. Thus this variable might help identifying repeated recording of water points. But some names seem to be generic, like 'school', which speaks against using this variable for the purpose of finding repeated recordings.
...
p1 <- univariate(data, 'extraction_type', flip=T)
getSummary(data$extraction_type_group, T)
p2 <- univariate(data, 'extraction_type_group', flip=T)
getSummary(data$extraction_type_class, T)
p3 <- univariate(data, 'extraction_type_class', flip=T)
...
p1 <- univariate(data, 'water_quality', flip=T)
getSummary(data$quality_group, T)
p2 <- univariate(data, 'quality_group', flip=T)
multiplot(p1, p2, cols=1)
```
...
p1 <- univariate(data, 'source', flip=T)
getSummary(data$source_type, T)
p2 <- univariate(data, 'source_type', flip=T)
getSummary(data$source_class, T)
p3 <- univariate(data, 'source_class', flip=T)
...
lineFrequency(datesubset$date_recorded,
              datesubset$status_group, 'Date Recorded', cor=T)
...
p <- lineFrequency(data$construction_year,
              data$status_group, 'Construction Year', cor=T)
...
## Difference in water point characteristics depending on location
...
### Differences in median construction year between locations
...
```{r fig.height=10}
multivariate(data$region, data$construction_year, data$status_group, log=F, c('region', 'construction_year', 'status_group'))
```

Two of the regions containing areas having a high amount of old water points, Rukwa and Mtwara, indeed have the earliest median construction years. The water points of Lindi on the other hand has a pretty average median age. And while the regions Shinyanga, Pwani, Manyara, which also have above average functional water points also have mostly newer water points, not so Arusha and Iringa, which have the highest fraction of functional water points. Thus the construction year seems to play a more important role in some regions than in others.
Differentiating in status group shows that non functional water points in every region have a higher median age than functional ones. The difference in both medians varies for the different regions. 

The same is true when grouping by basins:

```{r fig.height=10}
multivariate(data$basin, data$construction_year, data$status_group,log=F, c('basin', 'construction_year', 'status_group'))
```

## Dependency of extraction type used

## The influence of the extraction method and age on the waterpoint status

```{r fig.height=10}
multivariate(data$extraction_type_class, data$construction_year, data$status_group, log=F, c('extraction_type_class', 'construction_year', 'status_group'))
```
...
### The influence of extraction method and geographical height on waterpoint status

```{r fig.height=10}
multivariate(data$extraction_type_class, data$gps_height, data$status_group, log=F, c('extraction_type_class', 'gps_height', 'status_group'))
```
...
### Which extraction methods are used for which source types andwhat kinds of water points are used for these?

```{r fig.height=5}
bivariate(data$extraction_type_class, 
          data$source_type,
...
```{r fig.height=10}
multipie(data$extraction_type_class, data$public_meeting, data$status_group) + 
    scale_fill_manual(values = statusColor)
```
...
```{r fig.height=10}
multipie(data$extraction_type_class, data$payment_type, data$status_group) + 
    scale_fill_manual(values = statusColor)
```
...
As a first step NAs have to be identified and named accordingly. Since longitude and latitude are both only part of a coordinate, if one is an NA the other will be as well. To not over-represent the NAs, the longitude variable will be removed here.
...
lineFrequency(data$num_na, data$status_group, 'NA Count', cor=T)
...
p <- lineFrequency(data$num_na,
                   data$status_group, 
                   'NA Count') 
p + labs(x='NAs per entry',
         y='Frequency',
